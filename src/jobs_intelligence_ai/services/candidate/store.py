...
Backs the Saved-Jobs / Build-a-Candidate features with the pipeline tables in the
`Jobs_Intelligence_AI` schema (`config.APP_SCHEMA`). Austria and Slovakia share
the schema but use separate tables, kept apart by `config.TABLE_PREFIX` ("" for
Austria, "sk_" for Slovakia), replacing the old in-memory dicts in tabs/saved.py:
...
# Shared pipeline DB; per-country tables are kept apart by a table prefix
# (Austria unprefixed, Slovakia "sk_"). Resolved from the active profile at import
# time (COUNTRY is fixed for the life of the process), so AT and SK candidates
# land in separate tables and never mix.
_DB  = config.APP_SCHEMA
_PFX = config.TABLE_PREFIX

# Fully-qualified, per-country table names.
_T_CANDIDATE    = f"{_DB}.{_PFX}candidate"
_T_SAVED        = f"{_DB}.{_PFX}candidate_saved_job"
_T_COMPANY      = f"{_DB}.{_PFX}company"
_T_CAND_COMPANY = f"{_DB}.{_PFX}candidate_company"
_T_TARGET       = f"{_DB}.{_PFX}target_candidate"
_T_AUDIT        = f"{_DB}.{_PFX}audit_log"
...
_PROFILE_SCALARS = ("title", "experience_years", "location",
                    ...
                    # AI-analysed (gpt-5.5) fields — populated for LinkedIn imports
...
def get_or_create_candidate(name: str, profile: dict | None = None,
    ...
    with eng.begin() as conn:
        row = conn.execute(
            text(f"SELECT candidate_id FROM {_T_CANDIDATE} WHERE full_name = :n LIMIT 1"),
            {"n": name}).first()
        if row:
            cid = int(row[0])
            if profile:
        ...
        res = conn.execute(
            text(f"INSERT INTO {_T_CANDIDATE} ({keys}) VALUES ({vals})"), cols)
...
def _apply_profile(conn, candidate_id: int, profile: dict) -> None:
    ...
    conn.execute(text(f"UPDATE {_T_CANDIDATE} SET {sets} WHERE candidate_id = :cid"), cols)
...
def lookup_candidate(name: str) -> dict | None:
    ...
    sql = (f"SELECT c.full_name, c.status, c.is_template, "
           f"  (SELECT COUNT(*) FROM {_T_SAVED} s "
           f"   WHERE s.candidate_id = c.candidate_id) AS matches "
           f"FROM {_T_CANDIDATE} c WHERE c.full_name = :n LIMIT 1")
...
def get_profile(name: str) -> dict | None:
    ...
    with get_engine().connect() as conn:
        row = conn.execute(
            text(f"SELECT full_name, email, phone, linkedin, title, experience_years, location, "
                 ...
                 f"FROM {_T_CANDIDATE} WHERE full_name = :n LIMIT 1"),
    ...
    profile = {
        ...
        # AI-analysed fields (gpt-5.5) — null for CV/manual candidates
...
def _select_saved(where: str = "", params: dict | None = None) -> list[dict]:
    sql = (f"SELECT s.job_id, s.job_snapshot, s.score, s.grade, s.pipeline_status, "
           ...
           f"FROM {_T_SAVED} s "
           f"JOIN {_T_CANDIDATE} c ON c.candidate_id = s.candidate_id "
...
def list_candidates_detailed() -> list[dict]:
    ...
    sql = f"""
        ...
        FROM {_T_CANDIDATE} c
        LEFT JOIN {_T_SAVED} s ON s.candidate_id = c.candidate_id
...
def update_candidate(name: str, fields: dict, actor: str | None = None) -> bool:
    ...
    with get_engine().begin() as conn:
        ...
        if not row:
            return False
        conn.execute(text(f"UPDATE {_T_CANDIDATE} SET {sets} WHERE full_name = :n"), cols)
...
def update_saved_job(job_id: str, fields: dict, actor: str | None = None) -> dict | None:
    ...
    with get_engine().begin() as conn:
        row = conn.execute(
            text(f"SELECT candidate_id, extras FROM {_T_SAVED} "
        ...
        if sets:
            conn.execute(text(f"UPDATE {_T_SAVED} SET {', '.join(sets)} "
...
def delete_saved_job(job_id: str, actor: str | None = None) -> int:
    ...
    with get_engine().begin() as conn:
        row = conn.execute(
            text(f"SELECT candidate_id FROM {_T_SAVED} WHERE job_id = :jid LIMIT 1"),
            {"jid": job_id}).first()
        res = conn.execute(
            text(f"DELETE FROM {_T_SAVED} WHERE job_id = :jid"), {"jid": job_id})
        if row:
...
def delete_candidate(name: str, actor: str | None = None) -> bool:
    ...
    with get_engine().begin() as conn:
        ...
        if not row:
            return False
        cid = int(row[0])
        conn.execute(text(f"DELETE FROM {_T_CANDIDATE} WHERE candidate_id = :cid"), {"cid": cid})
...
def _get_or_create_company(conn, name: str, linkedin_url: str | None,
    ...
    if url:
        row = conn.execute(text(
            f"SELECT company_id FROM {_T_COMPANY} WHERE linkedin_url = :u LIMIT 1"),
            {"u": url}).first()
    else:
        row = conn.execute(text(
            f"SELECT company_id FROM {_T_COMPANY} "
    ...
    if row:
        ...
        if extra:   # backfill metadata we didn't have before
            ...
            conn.execute(text(f"UPDATE {_T_COMPANY} SET {assign} WHERE company_id = :id"),
                         {**extra, "id": cid})
        return cid
    ...
    keys = ", ".join(cols)
    vals = ", ".join(f":{k}" for k in cols)
    res = conn.execute(text(f"INSERT INTO {_T_COMPANY} ({keys}) VALUES ({vals})"), cols)
    return int(res.lastrowid)
    ...
    with get_engine().begin() as conn:
        for c in companies or []:
            ...
            exists = conn.execute(text(
                f"SELECT 1 FROM {_T_CAND_COMPANY} "
...
def list_companies() -> list[dict]:
    ...
    sql = (f"SELECT co.company_id, co.name, co.linkedin_url, co.industry, "
           ...
           f"FROM {_T_COMPANY} co "
           f"LEFT JOIN {_T_CAND_COMPANY} cc ON cc.company_id = co.company_id "
...
def save_target(draft: dict, label: str | None = None, created_by: str | None = None) -> int:
    ...
    with get_engine().begin() as conn:
        res = conn.execute(text(
            f"INSERT INTO {_T_TARGET} (label, draft, created_by) "
...
def _audit(conn, action: str, candidate_id: int | None, detail: str = "",
    ...
    conn.execute(text(
        f"INSERT INTO {_T_AUDIT} (actor, action, candidate_id, detail) "
