...
def generate_example_cv_pdf_sk() -> bytes:
    """Generate a sample CV PDF for Marek Novák, a Slovak software developer.

    The Slovak counterpart to the Max Weber example — used by the SK build so the
    'example PDF' flow shows a Bratislava-based candidate (Slovak market salary,
    Slovak languages/education) instead of an Austrian one.
    """
    buf = io.BytesIO()
    doc = SimpleDocTemplate(
        buf, pagesize=A4,
    ...
    s_name   = sty("namesk",  fontName="Helvetica-Bold", fontSize=22, textColor=NAVY, spaceAfter=3, leading=26)
    s_title  = sty("titlesk", fontSize=13, textColor=GREY, spaceAfter=2)
    s_contact= sty("contsk",  fontSize=10, textColor=GREY, spaceAfter=0)
    s_section= sty("secsk",   fontName="Helvetica-Bold", fontSize=10, textColor=NAVY, spaceBefore=14, spaceAfter=4)
    s_body   = sty("bodysk",  fontSize=10, leading=15, spaceAfter=4)
    s_jobt   = sty("jobtsk",  fontName="Helvetica-Bold", fontSize=10, spaceAfter=1)
    s_jobco  = sty("jobcosk", fontSize=10, textColor=GREY, spaceAfter=4)
    s_bullet = sty("bulsk",   fontSize=10, leading=15, leftIndent=10, spaceAfter=2)
    s_inline = sty("inlsk",   fontSize=10, leading=15, spaceAfter=4)

    story = [
        Paragraph("Marek Novák", s_name),
        Paragraph("Backend &amp; Full-Stack Developer", s_title),
        Paragraph("Bratislava, Slovakia  ·  marek.novak@example.sk  ·  +421 905 123 456", s_contact),
        Spacer(1, 0.3 * cm),
        HRFlowable(width="100%", thickness=2, color=NAVY, spaceAfter=0.4 * cm),
    ]
    ...
    story.append(Paragraph(
        "Software developer with 5 years of experience in backend and full-stack development. "
        "Specialised in Python/FastAPI REST APIs and React frontends, with hands-on experience "
        "deploying services via Docker and CI/CD pipelines. Ing. (MSc) in Informatics from "
        "FIIT STU Bratislava. Based in Bratislava; open to hybrid or on-site roles.",
        s_body,
    ))
    hr()
    ...
    YR_W = 3.0 * cm
    jobs = [
        ("2022 – present", "Backend Developer", "TechSolutions s.r.o., Bratislava",
         ["Designed and maintained REST APIs using Python and FastAPI.",
          "Managed PostgreSQL schema migrations and query optimisation.",
          "Containerised services with Docker; maintained CI/CD pipelines (GitLab CI).",
          "Participated in agile sprints and conducted regular code reviews."]),
        ("2020 – 2022", "Junior Developer", "WebFactory s.r.o., Bratislava",
         ["Built React/TypeScript frontends consuming REST and GraphQL APIs.",
          "Developed Node.js microservices for internal tooling.",
          "Contributed to sprint planning and technical documentation."]),
        ("2019 – 2020", "IT Intern", "Startup Hub Bratislava",
         ["Automated reporting workflows with Python scripts.",
          "Built internal dashboards using Flask and SQLite."]),
    ]
    for years, jobt, company, bullets in jobs:
    ...
    section("Education")
    edu = Table(
        [[Paragraph("2014 – 2019", s_contact),
          Paragraph("FIIT STU Bratislava — Ing. (MSc) Informatics", s_body)]],
        colWidths=[YR_W, COL_W - YR_W],
    )
    edu.setStyle(TableStyle([
        ("VALIGN",      (0, 0), (-1, -1), "TOP"),
    ...
    story.append(Paragraph(
        "Python  ·  FastAPI  ·  JavaScript / TypeScript  ·  React  ·  Node.js  ·  "
        "PostgreSQL  ·  Docker  ·  Git  ·  REST APIs  ·  Linux  ·  CI/CD (GitLab CI)",
        s_inline,
    ))
    hr()

    section("Languages")
    story.append(Paragraph("Slovak: Native  ·  Czech: Fluent  ·  English: C1 (Advanced)", s_inline))
    hr()

    section("Availability & Expectations")
    story.append(Paragraph(
        "Salary expectation: €2,800–3,600 gross/month  ·  Notice period: 2 months  ·  "
        "Work preference: Full-time permanent, Bratislava (hybrid/on-site)",
        s_inline,
    ))
