...
**Services — `services/enrichment/` (2.3 #2, +2.4 observation)** — offline `unit_tests/` (31): rescorer(6), highlighter(6), seniority(6), quality(5), match_insights(3), observation(5) — mock `responses.parse` (apply + clamp/skip + fallback); match_insights is pure (no LLM). `test_6_observation` covers the saved bp's HR profile-override chat: extract drops null override fields + grounds the profile in the prompt, no-key/error differs by call (extract raises; reply degrades to "Profile updated."), reply returns the model text or the fallback. Live `smoke_tests/` (6, asserts): rescorer, highlighter, seniority, quality, observation extract, observation reply.
...
**Services — `services/reporting/` (2.3 #4, +2.4 session_chat + company_summary)** — offline `unit_tests/` (21): `test_1_opportunity_briefing` (briefing returned shape-intact; `suggest_filters` drops options not in the provided lists; fallbacks on no-key / failure), `test_2_report_generator` (elaboration merged back by index; fallbacks; the pure PDF builder emits `%PDF`), `test_3_report_pipeline` (both PDF entry points emit `%PDF`, fed a real `build_insights` payload), `test_4_session_chat` (analytics + radar advisor chats return the reply string; saved items / market snapshot grounded into the instructions, history+message forwarded as `input`; no-key/null/error raise — these prose chats use a single-field `AdvisorReply`), `test_5_company_summary` (company hiring-profile summary returned + trimmed; no-key/null/error → "" since the summary is optional). Live `smoke_tests/test_reporting_smoke` (6, asserts): briefing sections, filter subset, per-insight elaboration, analytics chat, radar chat, company summary. `report_pipeline` is pure reportlab (no LLM).

**Services — `services/search/` (2.3 #5 single-job chat, +2.4 #9 match analysis)** — `unit_tests/test_job_chat` (4, offline): the single-job chat relocated from the old `chat.py`; inject a fake client to assert the reply, `previous_response_id` session continuity, candidate-text injection, and the no-key / error fallbacks. (The job-search chat `_parse` cases were removed from `shared/test_1_json` when that feature was deleted.) `unit_tests/test_match_analysis` (5, offline): the candidate-vs-matched-jobs assessment relocated from the search blueprint (SO single-field `MatchAnalysis`) — returns the prose text, grounds the profile + matched jobs into the input, and no-key / model-error / null-parse all raise. `smoke_tests/test_match_analysis_smoke` (1, live-asserting): a non-empty prose assessment for a candidate + matched jobs.

**Services — `services/clustering/` (2.3 #6, +2.4 segment_analysis)** — `unit_tests/` (15, offline): `test_1_persona` (Structured-Outputs persona; mock `responses.parse` + member fallback), `test_2_segmenting` (pure `cluster_labels` — empty/singleton edges + two-group separation), `test_3_embeddings` (mock `embeddings.create`; L2-normalization + zero-vector safety), `test_4_segment_chat` (reply, session continuity, no-key/error fallbacks), `test_5_segment_analysis` (SO cross-segment overview prose + per-job candidate grades in input order; inputs grounded into the prompt; no-key/null/error raise). `smoke_tests/test_clustering_smoke` (5, live-asserting): embed+cluster splits sellers vs nurses, persona synthesis, segment chat, segment overview, per-job grading.

**Services — `services/candidate/` (2.3 #7, +2.4 parser +2.4 guided_builder)** — `unit_tests/` (21, offline): `test_1_profile_enricher` (SO `LinkedInProfile` merge-over-base, empty-field guard, no-key/error → base), `test_2_assistant` (SO `CandidateReply`: reply→text, only the changed `profile_updates` fields, pure-discussion → no edits, session continuity, fallbacks), `test_3_example_cv` (all three sample-CV builders emit `%PDF`), `test_4_profile_parser` (SO `CandidateProfile`: parse→dict, no-key/null/error raise — the CV-text parser pulled out of the candidate blueprint in 2.4), `test_5_guided_builder` (SO `GuidedFieldUpdates`/`GuidedReply`: field extraction drops empty fields + grounds the catalog, reply→dict with `{value,label}` region options; extract returns {}/raises, reply always degrades to {} so the chat keeps working — the guided builder's two calls pulled out of the guided blueprint in 2.4). `smoke_tests/test_candidate_smoke` (6, live-asserting): enrichment, CV-text parse, assistant discussion (no edits), assistant CV edit, guided field extraction, guided reply phrasing. (The `_parse_candidate` cases were removed from `shared/test_1_json` when the assistant moved to Structured Outputs — that file now pins only `parse_json`.)

**Services — `services/job_detail/` (2.4 #8)** — `unit_tests/test_1_operations` (8, offline): the five Job-Detail-modal tools relocated from the job_detail blueprint — each prose tool (translate, compact, cv-questions, outreach) returns the model text + grounds its inputs (the English note appears only for `lang="en"`); `score_candidate_strength` returns a dict with the 5 dimensions in order; no-key / model-error / null-parse all raise. `smoke_tests/test_job_detail_smoke` (3, live-asserting): translate, compact, candidate-strength (five in-range scores + reasons + overall). The four prose tools share single-field `JobDetailText`; candidate-strength is the one real-JSON call (`CandidateStrength`) and killed the last "Return ONLY valid JSON" + `json.loads` in a blueprint.
