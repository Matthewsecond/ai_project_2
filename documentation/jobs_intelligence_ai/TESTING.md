...
**Default gate:** `pytest -m "not smoke"` → **158 passed, 27 deselected** — offline foundation + all repackaged services (stats, enrichment [+observation], interview, reporting [+analytics/radar session chats +company summary], search [grader + job_chat], clustering [+segment analysis], candidate [+CV-text parser +guided builder], geo, auth), plus the live search stability tests (the live-asserting smoke tests deselected).
