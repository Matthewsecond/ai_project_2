...
**Default gate:** `pytest -m "not smoke"` → **148 passed, 23 deselected** — offline foundation + all repackaged services (stats, enrichment, interview, reporting [+analytics/radar session chats +company summary], search [grader + job_chat], clustering [+segment analysis], candidate [+CV-text parser], geo, auth), plus the live search stability tests (the live-asserting smoke tests deselected).
