...
**Default gate:** `pytest -m "not smoke"` → **166 passed, 30 deselected** — offline foundation + all repackaged services (stats, enrichment [+observation], interview, reporting [+analytics/radar session chats +company summary], search [grader + job_chat], clustering [+segment analysis], candidate [+CV-text parser +guided builder], job_detail, geo, auth), plus the live search stability tests (the live-asserting smoke tests deselected).
