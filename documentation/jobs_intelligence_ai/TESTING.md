...
**Default gate:** `pytest -m "not smoke"` → **139 passed, 20 deselected** — offline foundation + all repackaged services (stats, enrichment, interview, reporting [+analytics/radar session chats], search [grader + job_chat], clustering, candidate [+CV-text parser], geo, auth), plus the live search stability tests (the live-asserting smoke tests deselected).
