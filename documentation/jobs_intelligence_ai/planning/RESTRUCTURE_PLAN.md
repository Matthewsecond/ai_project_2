...
| 2 | 🔶 `enrichment/` ← seniority_classifier, quality_classifier, match_insights, rescorer, highlighter | ✅ repointed (core, search orch, chat, search bp, saved bp) | 2a ✅ moved (+`__init__`/`config`); **2b rescorer**, **2c highlighter** → SO (drop `parse_json`, use `shared.get_client`); verify seniority/quality/match_insights |
